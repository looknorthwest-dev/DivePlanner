...
// Always use 'supabaseClient' to avoid conflicting with the window.supabase library object
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  tripReadout = document.getElementById('trip-readout');
...
function renderReadout() {
  ...
  if (!trip || !tripReadout) { if (tripReadout) tripReadout.hidden = true; return; }
...
function renderCardList({ containerId, rows, fieldDefs, table, titleKey, extraFooter }) {
  ...
  container.querySelectorAll('.card').forEach(cardEl => {
    ...
    cardEl.querySelector('.btn-delete').addEventListener('click', async () => {
      ...
      renderCardList({ containerId, rows, fieldDefs, table, titleKey, extraFooter });
    });
  });
  ...
  container.querySelectorAll('.btn-refresh-weather').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      if (!row.lat || !row.lng) {
        ...
        else { showToast('Add a name first'); }
      } else {
...
if (addStayBtn) {
  addStayBtn.addEventListener('click', async () => {
    const { data, error } = await supabaseClient.from('accommodations').insert({ trip_id: currentTripId, name: 'New Accommodation', position: stayRows.length }).select().single();
    if (error) { console.error(error); return; }
    stayRows.push(data);
    renderStays();
  });
